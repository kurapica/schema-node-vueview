...
export function useSingleView(
  ...
  if (node instanceof StructType)
    return !(
      !schemaViews[node.name.toLowerCase()] ||
      node
        .getFields()
        .some(
          (f) => f.type instanceof StructType || f.type instanceof ArrayType,
        )
    );
 ...
 */
export function getSubNodeFormType(
  node: DataNode,
  type?: SchemaNodeFormType,
  skinName: string = DEFAULT_SKIN,
): SchemaNodeFormType {
  return useSingleView(node.type, skinName) ||
    ...
    : type === SchemaNodeFormType.ExpandAll
      ...
      : type === SchemaNodeFormType.Expand2
        ? SchemaNodeFormType.Expand
        : type === SchemaNodeFormType.Expand3
          ? SchemaNodeFormType.Expand2
          : type
            ? SchemaNodeFormType.Nest
 ...
 */
export function regBaseSchemaKindView(
  kind: string,
  view?: any,
  resolve?: (node: DataNode, skin?: string) => any,
) {
 ...
 */
export function regSchemaTypeView(
  type: string,
  view: any,
  skinName: string = DEFAULT_SKIN,
  asSingle: boolean = false,
) {
  ...
  if (asSingle) singleView.add(key);
  else singleView.delete(key);
}

/**
 * Gets the view for the given schema node and skin
 */
export function getSchemaTypeView(
  node: DataNode,
  skinName: string = DEFAULT_SKIN,
) {
  // try registered view
  ...
  return (
    (baseMap.customResolve && baseMap.customResolve(node, skinName)) ||
    (baseMap.resolve && baseMap.resolve(node, skinName)) ||
    baseMap.view
  );
}

function getSchemaTypeViewBySchema(
  schema: IValueTypeAccess,
  skinName: string = DEFAULT_SKIN,
): any | undefined {
  skinName = skinName.toLowerCase();
  ...
  if (schema instanceof ScalarType)
    return schema.baseType
      ? getSchemaTypeViewBySchema(schema.baseType, skinName)
      : undefined;
  return undefined;
}
